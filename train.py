...
def save_flow_fields(inputs, outputs, timestep, fold, val, re, output_dir="output_files"):
    """
    Save original and reconstructed flow fields for a given timestep to files.
    
    Parameters:
        inputs (torch.Tensor): Original input flow fields (batch_size, channels, height, width).
        outputs (torch.Tensor): Reconstructed flow fields (batch_size, channels, height, width).
        timestep (int): The timestep to save data for.
    ...
    # Save each batch element as a separate file
    for i in range(inputs_np.shape[0]):  # Loop over batch
        print(f"Saving data for Re = ", re_np[i] * std_re + mean_re)
        re_value = int(re_np[i] * std_re + mean_re)  # Extract the Re value for the current input
        # re_value = int(re_np[i])  # Extract the Re value for the current input
        original_file = os.path.join(output_dir, f"{re_value}_original_{val}_f{fold}_b{i}_t{timestep}.npz")
        reconstructed_file = os.path.join(output_dir, f"{re_value}_reconstructed_{val}_f{fold}_b{i}_t{timestep}.npz")
        
        # Save original and reconstructed data
        np.savez(original_file, u_x=inputs_np[i, 0], u_y=inputs_np[i, 1], p=inputs_np[i, 2])
        np.savez(reconstructed_file, u_x=outputs_np[i, 0], u_y=outputs_np[i, 1], p=outputs_np[i, 2])

def save_flow_fields_seq(inputs, outputs, seq, re, output_dir="lstm_output_files"):
    """
    Save original and reconstructed flow fields for a given timestep to files.
    ...
    for i in range(inputs_np.shape[0]):
        # Denormalize the Reynolds number
        re_value = int(re_np[i] * std_re + mean_re)  # Convert to actual Re value
        print(f"Saving data for Re = {re_value}, Sequence = {seq}, Batch Element = {i}")

        # Define file paths for original and reconstructed data
        original_file = os.path.join(output_dir, f"{re_value}_original_s{seq}_b{i}.npz")
        reconstructed_file = os.path.join(output_dir, f"{re_value}_reconstructed_s{seq}_b{i}.npz")
    ...
    """
    print(f"Saving data for Re = ", re_np[0] * std_re + mean_re)
    re_value = int(re_np[0] * std_re + mean_re)  # Extract the Re value for the current input
    original_file = os.path.join(output_dir, f"{re_value}_original_s{seq}_.npz")
    reconstructed_file = os.path.join(output_dir, f"{re_value}_reconstructed_s{seq}_.npz")
    
    # Save original and reconstructed data
    np.savez(original_file, u_x=inputs_np[0, 0], u_y=inputs_np[0, 1], p=inputs_np[0, 2])
    np.savez(reconstructed_file, u_x=outputs_np[0, 0], u_y=outputs_np[0, 1], p=outputs_np[0, 2])
    """
